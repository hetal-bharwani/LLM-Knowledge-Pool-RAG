...
# document_to_embed = "knowledge_pool/AASZOK.txt"
...
# Directory containing the text files to embed
directory = "knowledge_pool"

# Iterate over each text file in the directory
for filename in os.listdir(directory):
    if filename.endswith(".txt"):
        document_to_embed = os.path.join(directory, filename)
        ...
        # Save the embeddings to a json file
        output_filename = os.path.splitext(document_to_embed)[0] + ".json"

        with open(output_filename, 'w', encoding='utf-8') as outfile:
            json.dump(embeddings, outfile, indent=2, ensure_ascii=False)

        print(f"Finished vectorizing. Created {output_filename}")
